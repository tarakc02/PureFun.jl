...
function addbm!(suite, Heap)
    ...
    suite[Heap]["ins_empty"] = @benchmarkable insert(h, x) setup=(h = $Heap{Int16}(); x = rand(Int16))
    suite[Heap]["ins_10k"] = @benchmarkable insert(h, x) setup=(h=randheap($Heap, 10_000); x=rand(Int16))
